...
def get_corr_filenames(output: str, directory: str = None) -> Tuple[str, str, str, str]:
    # d = dir if dir is not None else args.directory
...
def get_class_junc_filenames(output: str, dir: str) -> Tuple[str, str]:
    outputPathPrefix = os.path.join(dir, output)
    ...
    if os.path.exists(corrFASTA):
        ...
        if not gtf:
            ...
            with open(isoforms, "r") as isoforms_gtf:
                ...
                if ind == 0:
                    print(
                        f"WARNING: GTF has {isoforms} no annotation lines.",
                        file=sys.stderr,
                    )
            ...
            try:
                ...
                sys.stderr.write(f"ERROR: File {isoforms} without GTF/GFF format.")
                raise SystemExit(1)
                    ...
                    for line in isoforms_gtf:
                        if line[0] != "#":
                            ...
                            if chrom not in list(genome_dict.keys()):
                                sys.stderr.write(
                                    f"ERROR: gtf '{chrom}' chromosome not found in genome reference file."
                                )
                                sys.exit()
...
def sqanti3_qc(
    ...
    print(f"Reading genome fasta {genome}....", file=sys.stdout)
    ...
    if args.isoAnnotLite:
        if args.gff3:
            ISOANNOT_CMD = (
                "python "
                + ISOANNOT_PROG
                + f" {corrGTF} {outputClassPath} {outputJuncPath} -gff3 {args.gff3} -d {args.directory} -o {args.output}"
            )
        else:
            ISOANNOT_CMD = (
                "python "
                + ISOANNOT_PROG
                + f" {corrGTF} {outputClassPath} {outputJuncPath} -d {args.directory} -o {args.output}"
            )
...
def main():
    ...
    if args.directory is None:
        ...
        if os.path.isdir(args.directory):
            print(
                f"WARNING: output directory {args.directory} already exists. Overwriting!",
                file=sys.stderr,
            )
        else:
    ...
    args.doc = os.path.join(
        os.path.abspath(args.directory), args.output + ".params.txt"
    )
    ...
    if args.chunks == 1:
        sqanti3_qc(
            ...
            force_id_ignore=args.force_id_ignore,
            aligner_choice=args.aligner_choice,
            cage_peak=args.cage_peak,
            polyA_motif_list=args.polyA_motif_list,
            polyA_peak=args.polyA_peak,
            phyloP_bed=args.phyloP_bed,
            skipORF=args.skipORF,
            ...
            fl_count=args.fl_count,
            version=args.version,
            skip_report=args.skip_report,
            isoAnnotLite=args.isoAnnotLite,
            gff3=args.gff3,
        )
    else:
