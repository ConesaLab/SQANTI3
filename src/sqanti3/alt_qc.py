...
def split_input_run(args):
    if os.path.exists(SPLIT_ROOT_DIR):
        logging.error(f"WARNING: {SPLIT_ROOT_DIR} directory already exists! Abort!")
        sys.exit(-1)
    else:
    ...
    for i, (d, x) in enumerate(split_outs):
        logging.info(f"launching worker on on {x}....")
...
@click.option("-o", "--output", help="Prefix for output files.", required=False)
@click.option(
    "-d",
    "--dir",
...
def main():
    ...
    doc = os.path.join(os.path.abspath(directory), output + ".params.txt")
    logging.info(f"Write arguments to {doc}...")
    with open(doc, "w") as f:
        f.write("Version\t" + __version__ + "\n")
        f.write("Input\t" + os.path.basename(isoforms) + "\n")
        f.write("Annotation\t" + os.path.basename(annotation) + "\n")
        f.write("Genome\t" + os.path.basename(genome) + "\n")
        f.write("Aligner\t" + aligner_choice + "\n")
        f.write(
            "FLCount\t"
            + (os.path.basename(fl_count) if fl_count is not None else "NA")
            + "\n"
        )
        f.write(
            "Expression\t"
            + (os.path.basename(expression) if expression is not None else "NA")
            + "\n"
        )
        f.write(
            "Junction\t"
            + (os.path.basename(coverage) if coverage is not None else "NA")
            + "\n"
        )
        f.write(
            "CagePeak\t"
            + (os.path.basename(cage_peak) if cage_peak is not None else "NA")
            + "\n"
        )
        f.write(
            "PolyA\t"
            + (
                os.path.basename(polyA_motif_list)
                if polyA_motif_list is not None
                else "NA"
            )
            + "\n"
        ...
        f.write(
            "PolyAPeak\t"
            + (os.path.basename(polyA_peak) if polyA_peak is not None else "NA")
            + "\n"
        )
        f.write("IsFusion\t" + str(is_fusion) + "\n")
