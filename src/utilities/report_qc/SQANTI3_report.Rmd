...
# TODO: use render_ggplotly in every plot
render_ggplotly <- function(plot) {
  if (exists("plot") && !is.null(plot$data) && nrow(plot$data) > 0) {
    ggplotly(plot) %>% layout(showlegend = TRUE)
  } else {
    cat("Plot data is not available or is empty.")
  }
}
...
```{r, eval = exists("pSTM")}
# Check if the plot object exists and is non-empty
cat('\n###', "Supported Transcript Models {.tabset .tabset-fade .tabset-dropdown}", '\n')

cat('\n####', "Structural Classification", '\n')
if (exists("pSTM")) {
  render_ggplotly(pSTM)
}

if (exists("pSTM_perc")) {
  render_ggplotly(pSTM_perc)
}

cat('\n####', "Structural Subcategories FSM and ISM", '\n')
if (exists("pSTM.s1")) {
  render_ggplotly(pSTM.s1 + theme(axis.title.x = element_blank()))
}

if (exists("pSTM_perc.s1")) {
  render_ggplotly(pSTM_perc.s1 + theme(axis.title.x = element_blank()))
}

cat('\n####', "Structural Subcategories NIC and NNC", '\n')
if (exists("pSTM.s2")) {
  render_ggplotly(pSTM.s2 + theme(axis.title.x = element_blank()))
}

if (exists("pSTM_perc.s2")) {
  render_ggplotly(pSTM_perc.s2 + theme(axis.title.x = element_blank()))
}
...
if (length(p21.ISM.list)>0) {
  p21.stitles.ISM<-list("3' Fragment for ISM",
...
p22.stitles.ISM<-list("3' Fragment for ISM",
                      "Internal Fragment for ISM",
                      "5' Fragment for ISM",
                      "Intron Retention for ISM")
...
cat ('\n###',"PolyA Distance Analysis by subcategories {.tabset .tabset-fade .tabset-dropdown}", '\n')
cat ('\n####',"By FSM/ISM Subcategory" , '\n')
ggplotly(p.polyA_dist_subcat)%>%
  layout(margin = list(r = 130))
cat ('\n')

cat ('\n####',"By Non-FSM/ISM Subcategory", '\n')
...
cat('\n###',"By Structural Category {.tabset .tabset-fade .tabset-dropdown}", '\n')
cat('\n####','FSM and ISM', '\n')
render_ggplotly(p30.s1) 
cat ('\n####','NIC and NNC', '\n')
render_ggplotly(p30.s2)
cat ('\n####','Other Categories', '\n')
render_ggplotly(p30.s3) 
cat ('\n')
```
...
Coding potential analysis
  1. Psauron score among structural category
  2. Coding quailty per structural category
  3. ORF type per structural category
  4. Psauron score per ORF type
-->
```{r, eval=exists("p33")}
cat("## Coding potential {.tabset .tabset-fade}", '\n')
cat('### Psauron score by structural category {.tabset .tabset-fade}', '\n')
ggplotly(p33)%>%
  layout(margin = list(r = 130))%>%
  style(textposition="none")
cat ('\n')
...
```{r, eval=exists("p34")}
categories <- c("FSM","ISM","NNC","NIC","Genic Genomic",
                "Antisense","Fusion","Intergenic","Genic Intron")
cat("### Coding potential by structural category {.tabset .tabset-fade .tabset-dropdown}", '\n')
for (i in categories) {
  if (i %in% names(p34)){
  cat('#### ', i, '\n')
  print(htmltools::tagList(ggplotly(p34[[i]]) %>% layout(width=800, height=800, margin = list(r = 150))))
  cat ('\n')
  }
}
```
...
```{r,eval=exists("p35")}
cat("### ORF type by structural category {.tabset .tabset-fade}", '\n')
ggplotly(p35)
cat ('\n')
```

```{r, eval=exists("p36")}
cat("### Psauron score by ORF type {.tabset .tabset-fade}", '\n')
ggplotly(p36)%>%
  layout(margin = list(r = 130))
cat ('\n')
```
