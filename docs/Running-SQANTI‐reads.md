...
- **Replicate concordance (multi-axis)** — for samples sharing a design-factor level (replicates), how closely each agrees with its group on structural composition, read-length profile, and splice-site precision (1 = matches its replicates). A more sensitive check than UJC overlap. Shown only with a `--factor` that has a ≥2-replicate level.
