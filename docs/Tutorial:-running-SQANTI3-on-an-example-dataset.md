...
<!-- This section provides a brief overview of the example dataset to be utilized throughout this [folder](https://github.com/ConesaLab/SQANTI_examples/tree/main/example). The dataset consists of various files and directories that represent the output from different stages of the SQANTI3 pipeline. The requisite input files for running SQANTI3 are stored within this [directory](https://github.com/ConesaLab/SQANTI_examples/tree/main/data). -->
...
For an in-depth understanding of each parameter, you can delve into the detailed descriptions in the [filter wiki page](Running-SQANTI3-filter.md). The ML filter allows users to define TP and TN sets or utilize the built-in sets. After defining the sets, you'll split them for model training/testing. The ML filter will utilize the probability threshold to label transcripts as Isoforms or Artifacts.
...
```bash
sqanti3_rescue.py rules --filter_class example/rules_filter_output/UHR_chr22_RulesFilter_result_classification.txt \
