...
class TestRatioTSSReading:
    """Test the ratio_TSS_dict_reading function."""
    ...
    def test_ratio_tss_missing_isoforms(self, sample_isoforms_info):
        ...
        assert result["PB.103714.1"].ratio_TSS == 1
        assert result["PB.103724.1"].ratio_TSS == 1
        assert result["PB.103781.1"].ratio_TSS == 1
...
class TestProcessRTS:
    """Test the process_rts function."""
    
    def test_rts_detection_assignment(self, sample_isoforms_info, monkeypatch):
        """Test that RTS detection results are properly assigned."""
        from Bio.Seq import Seq
        from Bio.SeqRecord import SeqRecord
        
        # Create a mock genome dict to avoid file I/O
        genome_dict = {"chr22": SeqRecord(Seq("ATCG" * 100), id="chr22")}
        
        # Mock the rts function at module level to return controlled results
        def mock_rts(args, gd):
            return {
                "PB.124830.1": [(100, 200)],
                "PB.103724.1": [(300, 400)]
            }
        
        import src.qc_computations
        monkeypatch.setattr(src.qc_computations, "rts", mock_rts)
        
        result_info, rts_info = process_rts(
        ...
        # Isoforms in RTS_info should be marked TRUE
        assert result_info["PB.124830.1"].RTS_stage == "TRUE"
        assert result_info["PB.103724.1"].RTS_stage == "TRUE"
        
        # Isoforms not in RTS_info should be marked FALSE
        assert result_info["PB.103714.1"].RTS_stage == "FALSE"
        assert result_info["PB.103781.1"].RTS_stage == "FALSE"
        assert result_info["PB.23068.2"].RTS_stage == "FALSE"
        
        # RTS_info should be returned
        assert "PB.124830.1" in rts_info
        assert "PB.103724.1" in rts_info
    
    def test_rts_all_non_rts(self, sample_isoforms_info, monkeypatch):
        """Test when no isoforms have RTS."""
        from Bio.Seq import Seq
        from Bio.SeqRecord import SeqRecord
        ...
        # All should be FALSE since mock returns empty dict
        for iso in result_info:
            assert result_info[iso].RTS_stage == "FALSE"
        
        # RTS_info should be empty
        assert len(rts_info) == 0


class TestClassifyFSM:
    """Test the FSM classification function."""
...
class TestFullLengthQuantification:
    """Test the full-length quantification function."""
...
class TestIsoformsJunctions:
    """Test the isoforms_junctions function."""
