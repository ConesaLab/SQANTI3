...
def make_UJC_hash(args, df):
    ...
    for index, row in df.iterrows():
        ...
        # TODO: Change the file naming to use the standards of SQANTI3 via the helper fucntion
        introns_cmd = f"""gtftools -i {outputPathPrefix}tmp_introns.bed -c "$(cut -f 1 {outputPathPrefix}_corrected.cds.gff3 | sort | uniq | paste -sd ',' - | sed 's/chr//g')" {outputPathPrefix}_corrected.gtf.cds.gff"""
