...
metrics_values <- c(
  ...
  non_redundant_precision * 100  # Used twice to close the radar
)

metrics_data <- as.data.frame(matrix(metrics_values, nrow = 1))
colnames(metrics_data) <- c("Pre", "1/red", "FDR", "nrPre", "PDR", "Sen")

# Create interactive radar plot using plotly
interactive_radar_plot <- plot_ly(
  type = 'scatterpolar',
  r = metrics_values,
  theta = colnames(metrics_data),
  fill = 'toself',
  mode = 'lines+markers',
  marker = list(
    color = '#1f77b4',
  ...
  add_trace(
    hoverinfo = 'text',
    text = paste(colnames(metrics_data), ':', round(metrics_values, 2), '%')
  )
