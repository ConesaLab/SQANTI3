...
def checkFeatureInTranscript(dc_SQexons, dc_GFF3transExons, transSQ, transGFF3, start, end, strand, dc_SQcoding, dc_GFF3coding):
    #toTest
    transInterest = "ENST00000486627" #a veces problemas con las posiciones de la CDS en el classification 
    startInterest = 0
    global verbose
    bstart = False
...
def mappingFeatures(dc_SQexons, dc_SQcoding, dc_SQtransGene, dc_SQgeneTrans, dc_SQstrand, dc_GFF3exonsTrans, dc_GFF3transExons, dc_GFF3, dc_GFF3gene, dc_GFF3coding, dc_GFF3geneTrans, dc_geneID2geneName, dc_geneName2geneID, dc_GFF3transGene, filename, filename_prints):
    ...
    if SAVE_PROB_TRANSCRIPTS:
        file_novel_not_annot = open("file_novel_not_annot.txt","w+")
        file_not_gene_in_GFF3 = open("file_not_gene_in_GFF3.txt","w+")
        file_not_features_to_annotate = open("file_not_features_to_annotate.txt","w+")
    ...
    for transSQ in dc_SQexons.keys():
        ...
        elif(checkingNovel): #not info on "anyNovelAnnotation" and still is novel
            if(not transcriptFromReferenceWithNoAnnot): #is a real novel [transGFF3 and transSQ not found in reference]
                if SAVE_PROB_TRANSCRIPTS:
                    file_novel_not_annot.write(transSQ + "\n")
                novelTranscriptsNotAnnotated = novelTranscriptsNotAnnotated + 1
            else: #transcript novel without annotation
                if SAVE_PROB_TRANSCRIPTS:
                    file_not_gene_in_GFF3.write(transSQ + "\n")
                transcriptsNotAnnotatedNotGeneInformation = transcriptsNotAnnotatedNotGeneInformation +1
        else: #no hay ninguna anotación en el GFF3 por lo que el transcrito no es que no se anote, sino que no puede anotarse
            if SAVE_PROB_TRANSCRIPTS:
                file_not_features_to_annotate.write(transSQ + "\n")
            transcriptsAnnotated = transcriptsAnnotated + 1
    ...
    if SAVE_PROB_TRANSCRIPTS:
        file_novel_not_annot.close()
        file_not_gene_in_GFF3.close()
        file_not_features_to_annotate.close()
...
def main():
    ...
    parser.add_argument('-saveProbTranscripts', help='\t\Save problematic transcripts in three different files. Novel transcripts not annotated ("file_novel_not_annot.txt"). Not reference gene found in GFF3 reference ("file_not_gene_in_GFF3.txt"). Transcripts not annotated because any feature is transfered by genomic position ("file_not_features_to_annotate.txt").', required = False, action='store_true')
