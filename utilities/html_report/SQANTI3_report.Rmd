...
```{r p21.a_p21.b_p21.dist3.ISM.a_p21.dist3.ISM.b, eval = exists("p21.a") || exists("p21.b") || exists("p21.dist3.ISM.a") || exists ("p21.dist3.ISM.b") || exists ("p21.FSM.list.a") || exists ("p21.ISM.list.a") }
cat ('\n###', "Distance to Annotated Polyadenylation Site {.tabset .tabset-fade .tabset-dropdown}", '\n')
```

```{r, eval = exists("p21.FSM.list")}
if (length(p21.FSM.list) > 0) {
  ...
  for (i in 1:length(p21.FSM.list)) {
    ...
    if (!is.null((p21.FSM.list[[i]]))){
        print(htmltools::tagList(ggplotly(p21.FSM.list[[i]])%>%layout(width=700, margin = list(r = 150))))
    }
    if (!is.null((p21.FSM.list.a[[i]]))) {
        print(htmltools::tagList(ggplotly(p21.FSM.list.a[[i]])%>%layout(width=700, margin = list(r = 150))))
    }
  }
}
```

```{r p21.ISM.list, eval = exists("p21.ISM.list")}
...
if (length(p21.ISM.list)>0) {
  ...
  for (i in 1:length(p21.ISM.list)) {
    ...
    if (!is.null(p21.ISM.list[[i]])) {
          print(htmltools::tagList(ggplotly(p21.ISM.list[[i]]) %>% layout(width=700, margin = list(r = 150))))
    }
    if (!is.null(p21.ISM.list[[i]])) {
          print(htmltools::tagList(ggplotly(p21.ISM.list[[i]])%>% layout(width=700), margin = list(r = 150)))
...
```{r p22.a_p22.b_p22.dist3.ISM.a_p22.dist3.ISM.b, eval = exists("p22.a") || exists("p22.b") || exists("p22.dist5.ISM.a") || exists ("p22.dist5.ISM.b") || exists ("p22.FSM.list.a") || exists ("p22.ISM.list.a")}
cat ('\n###', "Distance to Annotated Transcription Start Site {.tabset .tabset-fade .tabset-dropdown}", '\n')
```

```{r p22.FSM.list, eval = exists("p22.FSM.list")}
if(length(p22.FSM.list)>0) {
  ...
  for (i in 1:length(p22.FSM.list)) {
    ...
    print(htmltools::tagList(ggplotly(p22.FSM.list[[i]])) %>%
            layout(width=700, margin = list(r = 150))
          )
...
```{r p22.ISM.list, eval = exists("p22.ISM.list")}
...
cat ('\n####', "For ISM", '\n')
if (exists("p22.dist5.ISM.a")) {
  ggplotly(p22.dist5.ISM.a) %>%
    layout(margin = list(r = 150))
  cat('\n') 
}

if (exists("p22.dist5.ISM.b")) {
...
if (exists("cage.FSM.list")) {
    ...
    for (i in 1:length(cage.FSM.list)){
      if (!is.null(cage.FSM.list[[i]])) {
              print(htmltools::tagList(ggplotly(cage.FSM.list[[i]])%>%layout(width=700, margin = list(r = 150))))

      }
      if (!is.null(cage.FSM.list.a[[i]])) {
              print(htmltools::tagList(ggplotly(cage.FSM.list.a[[i]])%>%layout(width=700, margin = list(r = 150))))
...
if (exists("cage.ISM.list")) {
    ...
    for (i in 1:length(cage.ISM.list)){
      if (!is.null(cage.ISM.list[[i]])) {
              print(htmltools::tagList(ggplotly(cage.ISM.list[[i]])%>%layout(width=700, margin = list(r = 200))))

      }
      if (!is.null(cage.ISM.list[[i]])) {
              print(htmltools::tagList(ggplotly(cage.ISM.list.a[[i]])%>%layout(width=700, margin = list(r = 200))))
      }
    }
  }
}
...
if (length(cage.NNC.list)>0) {
    for (i in 1:length(cage.NNC.list)){
      if (!is.null(cage.NNC.list[[i]])) {
        print(htmltools::tagList(ggplotly(cage.NNC.list[[i]])%>% layout(width=700, margin = list(r = 150))))
      }
      if (!is.null(cage.NNC.list.a[[i]])) {
        print(htmltools::tagList(ggplotly(cage.NNC.list.a[[i]])%>% layout(width=700, margin = list(r = 150))))
      }
    }
  }
...
```{r p30, eval=exists("p30") && exists("p30.a")}
cat ('\n###',"By Structural Category", '\n')
ggplotly(p30)
ggplotly(p30.a)
cat ('\n')
...
```{r, eval=(saturation.curves=='True')&&!all(is.na(data.class$FL))}
cat ('\n##',"Saturation curves", '\n')
...
ggplotly(p28.RTS)%>%
  layout(margin = list(r = 130)) %>%
  layout(showlegend = T) %>%
  style(textposition= 'top center', textfont = list(size=9))
